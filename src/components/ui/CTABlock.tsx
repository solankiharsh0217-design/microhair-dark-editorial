...
type Props = {
  ...
  className?: string;
  variant?: "whatsapp" | "phone" | "both";
};

export default function CTABlock({
  primaryHref = SITE.whatsapp,
  primaryLabel = "Invia le tue foto su WhatsApp — Preventivo in 24h",
  secondaryHref = SITE.phoneLink,
  ...
  return (
    <div
      className={`flex flex-col items-stretch gap-4 sm:flex-row sm:items-center ${justify} ${className}`}
    >
      {(variant === "whatsapp" || variant === "both") && (
        <a
          ...
          className="group inline-flex cursor-pointer items-center justify-center gap-3 rounded-full bg-[#25D366] px-7 py-4 text-xs font-semibold tracking-wider text-white uppercase transition-all hover:bg-[#1ebe5a]"
        >
